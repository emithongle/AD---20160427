...
from libs.utils import getGroupModels

def listFileInFolder(folder):
...
def loadModelS(ttype):
    return loadFile(folders['model'], timeManager.getTime() + '_' + files['model'][ttype])

def loadAllModel():
    modelConfigs = loadFile(folders['model'], files['model']['model-config'])

    models = {'name': {}, 'address': {}, 'phone': {}}
    for key in modelConfigs['name']:
        models['name'][key] = loadFile(folders['model'], key + '_' + 'model_name.pkl')
    for key in modelConfigs['address']:
        models['address'][key] = loadFile(folders['model'], key + '_' + 'model_address.pkl')
    for key in modelConfigs['phone']:
        models['phone'][key] = loadFile(folders['model'], key + '_' + 'model_phone.pkl')

    return {
        'name': getGroupModels(models['name'], modelConfigs['name']),
        'address': getGroupModels(models['address'], modelConfigs['address']),
        'phone': getGroupModels(models['phone'], modelConfigs['phone']),
    }
...
def saveTestStandardResults(data, dataSummary):
    data['summary'] = dataSummary
    saveFile(data, folders['result'], files['result']['model-test-result'])